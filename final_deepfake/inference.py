...
try:
    ...
    FLIP_LEFT_RIGHT = getattr(
        getattr(PILImage, "Transpose", PILImage), "FLIP_LEFT_RIGHT"
    )
except ImportError:
...
try:
    facer = importlib.import_module("facer")
except Exception:
...
def get_2d_sincos_pos_embed_cached(
...
class EnhancedTargetNetworkViT(nn.Module):
    def __init__(
        ...
        self.rep_decoder_pos_embed = nn.Parameter(
            torch.zeros(1, num_patches + 1, rep_decoder_embed_dim), requires_grad=False
        )
    ...
    def initialize_weights(self):
        ...
        self.rep_decoder_pos_embed.data.copy_(
            torch.from_numpy(rep_decoder_pos_embed).float().unsqueeze(0)
        )
...
class EnhancedFSFM_ArcFace(nn.Module):
    def __init__(
        ...
        if use_enhanced:
            ...
            raise ValueError(
                "use_enhanced=False is not supported because vit_target_network is unavailable"
            )

        self.head = nn.Sequential(
            nn.LayerNorm(256), nn.Linear(256, embed_dim), nn.LayerNorm(embed_dim)
        )
...
class FaceDetector:
    ...
    def __init__(self):
        self.dlib_detector = (
            dlib.get_frontal_face_detector() if dlib is not None else None
        )

        # OpenCV Haar Cascade fallback
        cv2_data = getattr(cv2, "data", None)
        cascade_base = cv2_data.haarcascades if cv2_data is not None else ""
...
class ImagePreprocessor:
    ...
    def crop_face(
        self, img: np.ndarray, margin_scale: float = 1.3
    ) -> Optional[np.ndarray]:
        """얼굴 영역 크롭"""
    ...
    def preprocess(
        ...
        img_pil = img_pil.resize(
            (self.target_size, self.target_size), RESAMPLING_LANCZOS
        )
...
class EnhancedDeepfakeDetector:
    ...
    def predict_with_tta(self, image: PILImage.Image) -> Dict[str, float]:
        ...
        # Horizontal flip
        img_flipped = image.transpose(FLIP_LEFT_RIGHT)
        ...
        for scale in [0.9, 1.1]:
            ...
            img_scaled = image.resize(new_size, RESAMPLING_LANCZOS)
            img_tensor = self.preprocessor.preprocess(
                img_scaled, use_face_detection=True
            )
            predictions.append(self.predict_single(img_tensor))

        # 평균 앙상블
        avg_real = float(np.mean([p["real_probability"] for p in predictions]))
        avg_fake = float(np.mean([p["fake_probability"] for p in predictions]))
        ...
        return {
            "real_probability": float(min(100.0, avg_real)),
            "fake_probability": float(min(100.0, avg_fake)),
        }

    def predict_image(
        self,
        image: Union[PILImage.Image, str, np.ndarray],
        use_face_detection: bool = True,
        ...
        try:
            ...
            # 라벨 결정
            predicted_label = (
                1 if result["fake_probability"] > result["real_probability"] else 0
            )
    ...
    def predict_batch(
        self, images: List[Union[PILImage.Image, np.ndarray]], batch_size: int = 8
